...
const REF = [
  ...
  // ── Mobile / Touch / Tilt ──
  { cat: 'Mobile', name: 'Touch input', sig: '(automatic)', desc: 'Touch events on the canvas automatically map to mouse_x/mouse_y, mouse_down, mouse_clicked, and mouse_released. No extra code needed for basic touch support.', example: '# Touch works like mouse!\nif game.mouse_clicked():\n    shoot(game.mouse_x(), game.mouse_y())' },
  { cat: 'Mobile', name: 'game.show_controls', sig: 'show_controls(layout="dpad_ab")', desc: 'Show on-screen touch controls. Layouts: "dpad_ab" (D-pad + A/B), "dpad" (D-pad only), "buttons" (A/B only). Virtual buttons feed into game.key_pressed().', example: 'game.show_controls("dpad_ab")\n\ndef update():\n    if game.key_pressed("left"):\n        player.x -= 5\n    if game.key_pressed("a"):\n        jump()\ngame.on_update(update)' },
  { cat: 'Mobile', name: 'game.hide_controls', sig: 'hide_controls()', desc: 'Remove on-screen touch controls.', example: 'game.hide_controls()' },
  { cat: 'Mobile', name: 'game.request_tilt', sig: 'request_tilt()', desc: 'Enable the accelerometer/tilt sensor. Required on iOS (requests permission). Call once at startup.', example: 'game.request_tilt()' },
  { cat: 'Mobile', name: 'game.tilt_x / tilt_y / tilt_z', sig: 'tilt_x() → float, tilt_y() → float, tilt_z() → float', desc: 'Read the device accelerometer. tilt_x = left/right, tilt_y = forward/back, tilt_z = up/down. Values typically range from -10 to 10.', example: 'game.request_tilt()\n\ndef update():\n    player.x += game.tilt_x()\n    player.y -= game.tilt_y()\ngame.on_update(update)' },
  ...
  // ── Scenes ──
  { cat: 'Scenes', name: 'game.on_scene', sig: 'on_scene(name, callback)', desc: 'Register a scene. The callback runs when you switch to this scene, resetting objects, tweens, timers, and camera.', example: 'def setup_menu():\n    title = game.Text("My Game", 200, 200, "white", 48)\n    btn = game.Button("Play", 250, 350)\n    btn.on_click(lambda o,x,y: game.scene("level1"))\ngame.on_scene("menu", setup_menu)' },
  { cat: 'Scenes', name: 'game.scene', sig: 'scene(name)', desc: 'Switch to a registered scene instantly. Clears all objects and runs the scene setup.', example: 'game.scene("menu")    # jump to menu\ngame.scene("level1")  # start level 1' },
  { cat: 'Scenes', name: 'game.scene_transition', sig: 'scene_transition(name, type="fade", duration=500, color="black")', desc: 'Switch to a scene with a visual transition. The screen covers, swaps at the midpoint, then reveals. Types: fade, wipe_left, wipe_right, wipe_down, wipe_up, circle, curtain, diagonal, blinds, pixelate, diamonds, squares.', example: 'game.scene_transition("level2", "fade", 600, "black")\ngame.scene_transition("boss", "circle", 800)\ngame.scene_transition("menu", "diamonds", 700)' },
  { cat: 'Scenes', name: 'game.get_scene', sig: 'get_scene() -> str', desc: 'Returns the name of the current scene.', example: 'if game.get_scene() == "menu":\n    game.scene("level1")' },
  ...
  { cat: 'Effects', name: 'game.transition', sig: 'transition(type, duration, color, on_mid, on_done)', desc: 'Standalone transition effect (no scene switch). Types: fade, wipe_left/right/down/up, circle, curtain, diagonal, blinds, pixelate, diamonds, squares. on_mid fires when fully covered, on_done fires when complete.', example: 'game.transition("pixelate", 800, "black")\n\n# with mid callback\ngame.transition("fade", 500, "black",\n    lambda: game.scene("level2"))' },
