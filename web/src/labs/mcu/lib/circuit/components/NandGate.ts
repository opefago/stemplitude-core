...
import { drawLogicNandIEC } from "../rendering/logicGateAndZenerSchematicDraw";

export interface NandGateProperties extends CircuitProperties {
  inputStates: [boolean, boolean];
  outputState: boolean;
}

/**
 * 2-input NAND gate (AND body + inversion bubble).
 */
export class NandGate extends CircuitComponent {
  protected gateProps: NandGateProperties;

  constructor(name: string, gridX: number = 0, gridY: number = 0) {
    const props: NandGateProperties = {
      value: 0,
      tolerance: 0,
      powerRating: 0.1,
      voltage: 5,
    ...
    super(name, "nand_gate", props, gridX, gridY);
    this.gateProps = props as NandGateProperties;
  }
