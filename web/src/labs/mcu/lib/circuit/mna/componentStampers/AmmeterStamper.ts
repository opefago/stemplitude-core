...
export class AmmeterStamper implements ComponentStamper {
  public readonly type = "ammeter";
  ...
  public stamp(component: CircuitComponent, ctx: ComponentStamperContext): void {
    ctx.netlist.add({
      ...
      apply: (vs) => {
        const resistance = (component.getCircuitProperties() as any).resistance || 0.001;
