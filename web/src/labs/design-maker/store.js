...
export const SHAPE_DEFAULTS = {
  ...
  sphere: {
    geometry: { radius: 10, widthSegments: 32, heightSegments: 32 },
    color: "#ec4899",
  ...
  torus: {
    geometry: { radius: 10, tube: 3, radialSegments: 16, tubularSegments: 48 },
    color: "#8b5cf6",
    flat: true,
    icon: "M8 4C4.1 4 1 5.8 1 8s3.1 4 7 4 7-1.8 7-4-3.1-4-7-4z M8 6c1.7 0 3 .9 3 2s-1.3 2-3 2-3-.9-3-2 1.3-2 3-2z",
  },
  text: {
    geometry: { text: "Hello", size: 10, height: 5, font: "helvetiker" },
    color: "#3b82f6",
    flat: true,
    isText: true,
  ...
  pyramid: {
    geometry: { radius: 10, height: 20, radialSegments: 4 },
    color: "#22c55e",
    icon: "M8 2L2 12h12z M8 2l4.5 8 M8 2L3.5 10",
  },
  heart: {
    geometry: { size: 10, depth: 5 },
    color: "#ef4444",
    flat: true,
  ...
  star: {
    geometry: { outerRadius: 10, innerRadius: 5, points: 5, depth: 5 },
    color: "#eab308",
    flat: true,
  ...
  ring: {
    geometry: { outerRadius: 10, innerRadius: 6, height: 3 },
    color: "#ca8a04",
    flat: true,
  ...
  starSix: {
    geometry: { outerRadius: 10, innerRadius: 5, points: 6, depth: 5 },
    color: "#d97706",
    flat: true,
...
export const useDesignStore = create((set, get) => ({
  ...
  duplicateSelected: () => {
    ...
    const extraGroups = [...groupMap.values()].map((id, i) => ({
      id,
      name: `Group Copy ${i + 1}`,
    }));
    set({
  ...
  arraySelected: (axis, count, spacing) => {
    ...
    const extraGroups = [...new Set([...groupMap.values()])].map((id, i) => ({
      id,
      name: `Array Group ${i + 1}`,
    }));
    set({
      objects: nextObjects,
      groups: reconcileGroups(nextObjects, [...state.groups, ...extraGroups]),
  ...
  selectObject: (id, addToSelection = false) =>
    set((state) => {
      ...
      const targetIds = obj?.groupId
        ...
        : [id];
      if (addToSelection) {
        const allSelected = targetIds.every((tid) =>
          state.selectedIds.includes(tid),
        );
        if (allSelected) {
          return {
            selectedIds: state.selectedIds.filter(
              (sid) => !targetIds.includes(sid),
            ),
          };
        }
        return {
          selectedIds: [...new Set([...state.selectedIds, ...targetIds])],
        };
      }
  ...
  groupSelected: () => {
    ...
    const groups = reconcileGroups(objects, [
      ...state.groups,
      { id: groupId, name: `Group ${state.groups.length + 1}` },
    ]);
    set({
      objects,
      groups,
  ...
  clearSelection: () =>
    set({ selectedIds: [], mirrorMode: false, workplaneMode: false }),
  selectAll: () =>
    set((state) => ({ selectedIds: state.objects.map((o) => o.id) })),
  setTransformMode: (mode) => set({ transformMode: mode }),
  toggleMirrorMode: () => set((state) => ({ mirrorMode: !state.mirrorMode })),
  setMirrorMode: (v) => set({ mirrorMode: !!v }),
  toggleWorkplaneMode: () =>
    set((state) => ({ workplaneMode: !state.workplaneMode })),
  ...
  alignObjects: (axis, edge) => {
    ...
    let target;
    if (edge === "min")
      target = Math.min(...selected.map((o) => o.position[ai]));
    else if (edge === "max")
      target = Math.max(...selected.map((o) => o.position[ai]));
    else
      target =
        selected.reduce((s, o) => s + o.position[ai], 0) / selected.length;
    set({
      objects: state.objects.map((o) => {
  ...
  mirrorSelected: (axis) => {
    ...
    const selected = state.objects.filter((o) => effective.includes(o.id));
    const center =
      selected.reduce((sum, o) => sum + o.position[ai], 0) /
      (selected.length || 1);
    set({
      objects: state.objects.map((o) => {
        if (!effective.includes(o.id)) return o;
        ...
        if (geometry.bufferGeometry) {
          try {
            return {
              ...rest,
              geometry: {
                bufferGeometryJSON: geometry.bufferGeometry.toJSON(),
              },
            };
          } catch {
      ...
      if (o.type === "imported" && o.geometry.bufferGeometryJSON) {
        try {
          ...
          return {
            ...o,
            geometry: {
              bufferGeometry: loader.parse(o.geometry.bufferGeometryJSON),
            },
          };
        } catch {
          return o;
