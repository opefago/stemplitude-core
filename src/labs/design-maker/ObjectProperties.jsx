...
function GeometryProps({ obj, updateGeometry }) {
  ...
  switch (obj.type) {
    ...
    case 'star':
      return (
        <>
          <NumberInput label="Outer R" value={p.outerRadius} onChange={(v) => updateGeometry('outerRadius', v)} />
          <NumberInput label="Inner R" value={p.innerRadius} onChange={(v) => updateGeometry('innerRadius', v)} />
          <NumberInput label="Points" value={p.points} onChange={(v) => updateGeometry('points', Math.max(3, Math.floor(v)))} step={1} />
          <NumberInput label="Depth" value={p.depth} onChange={(v) => updateGeometry('depth', v)} />
        </>
      );
    default:
    ...
    <div className="dml-properties">
      ...
      <div className="dml-prop-section">
        ...
        <div className="dml-prop-row">
          <NumberInput label="X" value={obj.position[0]} onChange={(v) => updatePos(0, v)} />
          <NumberInput label="Y" value={obj.position[1]} onChange={(v) => updatePos(1, v)} />
          <NumberInput label="Z" value={obj.position[2]} onChange={(v) => updatePos(2, v)} />
        </div>
      </div>
      ...
      <div className="dml-prop-section">
        <h4>Rotation</h4>
        <div className="dml-prop-row">
          <NumberInput label="X" value={(obj.rotation[0] * 180) / Math.PI} onChange={(v) => updateRot(0, v)} step={5} />
          <NumberInput label="Y" value={(obj.rotation[1] * 180) / Math.PI} onChange={(v) => updateRot(1, v)} step={5} />
          <NumberInput label="Z" value={(obj.rotation[2] * 180) / Math.PI} onChange={(v) => updateRot(2, v)} step={5} />
        </div>
      </div>
      ...
      <div className="dml-prop-section">
        ...
        <div className="dml-prop-row">
          ...
          <NumberInput label="Z" value={obj.scale[2]} onChange={(v) => updateScale(2, v)} step={0.1} min={0.01} />
        </div>
      </div>
      ...
      <div className="dml-prop-section">
        ...
        <label className="dml-checkbox-label">
          <input
            type="checkbox"
            checked={obj.isHole}
            onChange={(e) => updateObject(obj.id, { isHole: e.target.checked })}
          />
